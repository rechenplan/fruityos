...
if [ "$#" -ne 3 ]; then
    echo "usage: test-link-linux.sh PHOBOS ORGASM PAD" >&2
    exit 2
fi
...
tmp=${TMPDIR:-/tmp}/phobos-link-test-$$
trap 'rm -rf "$tmp"' EXIT INT TERM
mkdir -p "$tmp"

"$phobos" -o 65792 -M "$tmp/sol.map" "$here/link.sol"
