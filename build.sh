...
for program in del jar juicer orgasm pish; do
...
cat "$root/seed/bin/fdseed.bin" "$root/initrd.jar" \
    > "$root/fruityos_floppy.img"
floppy_size=$(stat --printf="%s" "$root/fruityos_floppy.img")
if test "$floppy_size" -gt 350208; then
    echo "build.sh: floppy image exceeds its 342 KiB load window" >&2
    exit 1
fi
truncate -s 1440K "$root/fruityos_floppy.img"
